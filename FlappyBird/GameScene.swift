...
//  FlappyBird
//
//  Created by 津端 俊尚 on 2022/04/21.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupItem() {
        ...
        // アイテム位置の上下の振れ幅を10ptとする
        let random_y_range: CGFloat = 10
        ...
        let createItemAnimation = SKAction.run({
            // 空の中央位置にランダム値を足して、りんごの表示位置を決定する
            let random_y = CGFloat.random(in: -random_y_range...random_y_range)
            let item_y = sky_center_y + random_y
            
            // りんごを作成
            let appleNode = SKSpriteNode(texture: itemTexture)
            appleNode.position = CGPoint(x: self.frame.size.width + itemTexture.size().width / 2, y: item_y)
            appleNode.zPosition = -50 // 雲より手前、地面より奥
            
            // りんごに物理体を設定する
            appleNode.physicsBody = SKPhysicsBody(rectangleOf: itemTexture.size())
            appleNode.physicsBody?.categoryBitMask = self.itemCategory    // ←追加
            appleNode.physicsBody?.isDynamic = false
            
            // アイテムをまとめるノードにアニメーションを設定
            appleNode.run(itemAnimation)
            
            // アイテムを表示するノードに今回作成したりんごを追加
            self.itemNode.addChild(appleNode)
        })
